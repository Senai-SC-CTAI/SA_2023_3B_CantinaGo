...
public class ComidaController {
  ...
  public List<Comida> listarComidas() {
   ...
   @GetMapping("/{nome}")
  public ArrayList<Comida> listarComidas(@PathVariable String nome) {
    ArrayList<Comida> filtro = new ArrayList<Comida>();
    for(Comida encontrada : comidaRepository.findAll()){
      if(encontrada.getNome().contains(nome)){
        filtro.add(encontrada);
      }
    }
    return filtro;
  }
  ...
  public ResponseEntity<String> deletarComida(@PathVariable Long id) {
    ...
    public ResponseEntity<Comida> atualizarComida(@PathVariable Long id, @RequestBody Comida comidaAtualizado) {
        if (comidaRepository.existsById(id)) {
            ...
            comida.setData(comidaAtualizado.getData());
